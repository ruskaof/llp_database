...
void init_new_element_offsets_for_deleted_element(const uint64_t new_element_offset) {
    ...
    // init for rb tree
    struct ElementHeader *rb_tree_element = (struct ElementHeader *) ((char *) get_file_data_pointer() +
    ...
    if (rb_tree->max_size == rb_tree->elements_count) {
        uint64_t prev_size = rb_tree_element->element_size;

        uint64_t new_rb_tree_offset;
        allocate_element(
                prev_size * 2 * sizeof(struct RbElement) + sizeof(struct RbTree) + sizeof(struct ElementHeader),
                ET_RB_TREE, &new_rb_tree_offset);
        ...
        delete_element(file_header->rb_tree_offset);
...
void init_new_element_offsets_for_rb_tree(const uint64_t offset) {
    ...
    struct RbTree *rb_tree = (struct RbTree *) ((char *) get_file_data_pointer() + offset +
                                                sizeof(struct ElementHeader));
    rb_tree->elements_count = 0;
    rb_tree->max_size = (rb_tree_header->element_size - sizeof(struct ElementHeader) - sizeof(struct RbTree)) /
                        sizeof(struct RbElement);
}
